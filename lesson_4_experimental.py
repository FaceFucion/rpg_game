...
class Medic(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        ...
        print(f'Medic {self.name} healed heroes by {self.__heal_points} points.')
...
class Hacker(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        if self.__can_steal:
            ...
            print(f'Hacker {self.name} stole {self.__steal_amount} health from boss and gave it to {hero.name}.')
        self.__can_steal = not self.__can_steal
...
def start_game():
    ...
    medic = Medic(name='Aibolit', health=250, damage=5, heal_points=15)
    witcher = Witcher(name='Geralt', health=200, damage=0)
    hacker = Hacker(name='Neo', health=240, damage=5, steal_amount=30)
    heroes_list = [warrior_1, medic, warrior_2, magic, berserk, witcher, hacker]
